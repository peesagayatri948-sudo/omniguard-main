...
export default function AiRemediation() {
  ...
  const handleLaunchOrchestrator = async () => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:5175/launch-claude-orchestrator', {
        method: 'POST',
  ...
  const handleDownloadZip = () => {
    window.location.href = `http://127.0.0.1:5175/download-repo?repoName=omniguard-enterprise`;
  };
  ...
  const handleExplainFinding = async (individual: Finding) => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:5175/explain-finding', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleGeneratePatch = async (individual: Finding) => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:5175/generate-patch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleApplyFix = async (individual: Finding) => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:5175/ai-fix', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          orgId,
  ...
  const handleGitCommit = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5175/git-commit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          repoName: 'omniguard-enterprise',
  ...
  const handleExportMarkdown = async () => {
    try {
      const res = await fetch('http://127.0.0.1:5175/rules');
      const data = await res.json();
