...
export default function CloudDrift() {
  ...
  const [sandboxFile, setSandboxFile] = useState<{ name: string; content: string } | null>(null);
  const [sandboxFindings, setSandboxFindings] = useState<any[]>([]);
  ...
  const handleDriftAutoFix = async (drift: ActiveDrift) => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:5175/drift-auto-fix', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Custom HCL File Sandbox Scanner */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center gap-2 mb-5">
          <Cloud size={16} className="text-purple-600" />
          <h2 className="text-sm font-semibold text-gray-900">Custom HCL File Sandbox Scanner</h2>
        </div>
        <p className="text-xs text-gray-500 mb-4">
          Select or drag a local <code>.tf</code> configuration file to run a one-time sandbox compliance scan. OmniGuard will run structural graph checks and offer AI-remediated code options.
        </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <input
              ...
              onChange={async (e) => {
                ...
                reader.onload = async (evt) => {
                  ...
                  setSandboxFile({ name: file.name, content });
                  // Trigger scan immediately
                  setScanLoading(true);
                  setSandboxFindings([]);
                  setFixedCode(null);
                  try {
                    const port = window.location.port === '5183' ? '5185' : '5175';
                    const res = await fetch(`http://127.0.0.1:${port}/api/scan-hcl`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                    ...
                    const data = await res.json();
                    if (data.findings) {
                      setSandboxFindings(data.findings);
                    } else {
            ...
            />
            <label
              htmlFor="hcl-file-picker"
              className="btn-primary text-xs px-4 py-2 cursor-pointer flex items-center gap-2"
            >
              Select .tf Configuration File
            </label>
            {sandboxFile && (
              <span className="text-xs text-gray-600 font-mono">
                Selected: {sandboxFile.name} ({sandboxFile.content.split('\n').length} lines)
              </span>
            )}
          </div>

          {scanLoading && (
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg text-center text-xs text-gray-500 animate-pulse">
              Parsing HCL tokens & running security graph audits...
            </div>
          )}

          {sandboxFile && !scanLoading && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
              {/* Left Side: Original Code & Findings */}
              <div className="space-y-4">
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="bg-gray-50 border-b border-gray-200 px-4 py-2 text-xs font-semibold text-gray-700 flex justify-between items-center">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          const port = window.location.port === '5183' ? '5185' : '5175';
                          const res = await fetch(`http://127.0.0.1:${port}/api/fix-hcl`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                              content: sandboxFile.content,
                              filePath: sandboxFile.name,
                              findings: sandboxFindings
                            })
                          });
                          const data = await res.json();
                          if (data.fixedContent) {
                            setFixedCode(data.fixedContent);
                          } else {
                      ...
                      disabled={fixLoading || sandboxFindings.length === 0}
                      className="px-2 py-1 bg-purple-600 text-white rounded text-[10px] font-bold disabled:bg-gray-300 hover:bg-purple-700 transition-colors"
                    >
                      {fixLoading ? 'Applying fixes...' : '✨ AI Auto-Fix File'}
                    </button>
                  </div>
                  <pre className="p-4 bg-gray-900 text-gray-100 text-xs font-mono overflow-auto max-h-[300px]">
                    {sandboxFile.content}
                  </pre>
                </div>

                <div className="space-y-2">
                  <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wide">Sandbox Scan Findings ({sandboxFindings.length})</h3>
                  {sandboxFindings.length === 0 ? (
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-xs text-green-700 font-semibold">
                      ✓ No infrastructure vulnerabilities or drifts found.
                    </div>
                  ) : (
                    <div className="space-y-2 max-h-[250px] overflow-y-auto">
                      {sandboxFindings.map((f: any, idx: number) => (
                        <div key={idx} className="p-3 bg-red-50 border border-red-200 rounded-lg">
                          ...
                          <h4 className="text-xs font-semibold text-gray-900 mt-1">{f.title}</h4>
                          <p className="text-[11px] text-gray-600 mt-0.5">{f.description}</p>
                          <p className="text-[10px] text-gray-400 mt-1 font-mono">Evidence: {f.evidence}</p>
                          {f.clause_reference && (
                            <span className="inline-block mt-1 text-[10px] bg-blue-50 text-blue-700 border border-blue-100 px-1.5 py-0.5 rounded font-medium">
              ...
              <div>
                <div className="border border-gray-200 rounded-lg overflow-hidden h-full flex flex-col">
                  <div className="bg-gray-50 border-b border-gray-200 px-4 py-2 text-xs font-semibold text-gray-700 flex justify-between items-center">
                    <span>Remediated Secure Config</span>
                    {fixedCode && (
                      <button
                        onClick={() => {
                          ...
                          a.href = url;
                          a.download = sandboxFile.name.replace('.tf', '_secured.tf');
                          a.click();
                        }}
                        className="px-2 py-1 bg-green-600 text-white rounded text-[10px] font-bold hover:bg-green-700 transition-colors"
                      >
                        Download Secured TF
                      </button>
                    )}
                  ...
                  {fixedCode ? (
                    <pre className="p-4 bg-gray-900 text-green-400 text-xs font-mono overflow-auto flex-1 max-h-[580px]">
                      {fixedCode}
                    </pre>
                  ) : (
                    <div className="flex-1 min-h-[300px] flex items-center justify-center bg-gray-50 text-xs text-gray-400 p-6 text-center">
                      {fixLoading ? 'Generating secured HCL configuration...' : 'Run "AI Auto-Fix File" to see corrected configuration options.'}
                    </div>
                  )}
                </div>
              </div>
