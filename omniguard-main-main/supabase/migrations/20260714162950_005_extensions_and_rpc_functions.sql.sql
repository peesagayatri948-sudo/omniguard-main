...
-- Enable pgvector for policy chunk embeddings
CREATE EXTENSION IF NOT EXISTS vector;

-- Add embedding column to policy_chunks (was omitted in migration 003 due to vector type)
ALTER TABLE policy_chunks ADD COLUMN IF NOT EXISTS embedding vector(1536);

-- ============================================================================
-- FUNCTION: check_rate_limit
-- Used by api-gateway and scan-worker edge functions
-- ============================================================================
...
BEGIN
  ...
  ON CONFLICT (key) DO UPDATE
    SET count = rate_limit_counters.count + 1,
        window_start = CASE
          WHEN rate_limit_counters.window_start < v_window THEN now()
...
-- ============================================================================
-- FUNCTION: claim_next_scan
-- Used by scan-worker to atomically claim the next queued scan
-- ============================================================================
...
BEGIN
  ...
  IF v_scan IS NOT NULL THEN
    RETURN QUERY SELECT
      v_scan.id,
      v_scan.organization_id,
      v_scan.repository_id,
      v_scan.scan_type,
      v_scan.branch,
      v_scan.trigger,
      v_scan.metadata;
...
-- ============================================================================
-- FUNCTION: match_policy_chunks
-- Used by scan-worker for RAG policy similarity search
-- ============================================================================
...
-- Helper for frontend to fetch orgs a user belongs to
-- ============================================================================
...
-- ============================================================================
-- FUNCTION: get_org_member_role
-- Helper for edge functions to resolve caller role
-- ============================================================================
CREATE OR REPLACE FUNCTION get_org_member_role(
  p_org_id  uuid,
  p_user_id uuid
)
RETURNS member_role
