...
async function main() {
  ...
  const vulnerabilities = await fetchFromJsonEndpoint(`http://127.0.0.1:5185/orchestrator/vulnerabilities?repoName=${encodeURIComponent(repoName)}`);
  const architecture = await fetchFromJsonEndpoint(`http://127.0.0.1:5185/orchestrator/context?repoName=${encodeURIComponent(repoName)}`);
...
- If accepted, call 'approve_modifications' to mark "Ready".`;
  ...
  try {
    ...
    let commandStr = `${npxCmd} -y @anthropic-ai/claude-code --dangerously-skip-permissions`;
    if (apiKey) {
      commandStr += ` -p "Read instructions from .omniguard_prompt.txt"`;
    }
    const claudeProcess = spawn(commandStr, [], {
      cwd: workspacePath,
      stdio: 'inherit',
    ...
    claudeProcess.on('close', (code) => {
      ...
      try {
        ...
        execSync(`curl -X POST http://127.0.0.1:5185/api/manual-scan -H "Content-Type: application/json" -d "{\\"repoName\\":\\"${repoName}\\"}"`, { stdio: 'ignore' });
