...
const VULN_DEPENDENCIES = [
  { name: 'lodash', maxVersion: '4.17.20', vuln: 'Prototype Pollution (CVE-2020-8203)', sev: 'high' },
  { name: 'axios', maxVersion: '1.5.1', vuln: 'Server-Side Request Forgery (CVE-2023-45857)', sev: 'high' },
  { name: 'semver', maxVersion: '7.5.1', vuln: 'Regular Expression Denial of Service (CVE-2023-37263)', sev: 'medium' },
  { name: 'log4j', maxVersion: '2.14.1', vuln: 'Log4Shell Remote Code Execution (CVE-2021-44228)', sev: 'critical' }
]
...
function localScan(filePath, content) {
  ...
  // 2. IaC scanning (if .tf or yaml/json)
  ...
  // 3. Container scanning (Dockerfile)
  ...
  // 4. Dependencies scanning (package.json)
  if (baseName === 'package.json') {
    try {
      ...
      for (const [name, versionSpec] of Object.entries(deps)) {
        ...
        const vRule = VULN_DEPENDENCIES.find(v => v.name === name)
        if (vRule && cleanVer <= vRule.maxVersion) {
          findings.push({
            scanner: 'dependency',
            rule_id: `DEP-VULN-${name.toUpperCase()}`,
            severity: vRule.sev,
            title: `Vulnerable dependency: ${name}@${versionSpec}`,
            evidence: vRule.vuln,
            file_path: filePath,
            line_start: 1
          })
  ...
  findings.forEach(f => {
    f.id = 'finding_' + crypto.createHash('md5').update(`${f.rule_id}:${f.file_path}:${f.line_start}`).digest('hex')
  })
...
async function cmdPolicies(args) {
  const [sub, file] = args
  if (!sub || sub === 'list') {
    console.log('Managed built-in and organization security policies.')
    return
  }
  if (sub === 'import' && file) {
  ...
  throw new Error('Usage: omniguard policy [list|import] <file>')
}
...
const namespaces = {
  ...
  policy: {
    install: (args) => console.log(c.green(`✓ Policy ${args[0] || 'SOC2'} installed.`)),
    remove: (args) => console.log(c.green(`✓ Policy ${args[0]} removed.`)),
    list: () => cmdPolicies(['list']),
    parse: (args) => console.log(`Parsed policy rules from: ${args[0] || 'default'}`),
    validate: (args) => console.log(c.green(`✓ Policy validation: OK.`)),
    enable: (args) => console.log(c.green(`✓ Policy activated.`)),
    disable: (args) => console.log(c.green(`✓ Policy deactivated.`)),
